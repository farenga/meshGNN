function [Tau_fine] = generate_fine_mesh_CartesianIntersection(Tau_coarse,Nit)

DatiHere = dati_2;
DatiHere.type_mesh = 'QUAD';
DatiHere.method = 'LDG';
DatiHere.basis = 'modal';
[Tau_cartesian] = generate_mesh_2(DatiHere,Nit);
...
%Tau_cartesian = generate_mesh_L_shaped(@MbbDomain_L_shaped,100,100);
...
for ie = 1:Tau_coarse.ne
    ...
    a = union( find( Tau_cartesian.BBox(:,1) > xH_max ), find( Tau_cartesian.BBox(:,2) < xH_min ) );
    b = union( find( Tau_cartesian.BBox(:,3) > yH_max ), find( Tau_cartesian.BBox(:,4) < yH_min ) );
    inter_element = setdiff( [1:1:Tau_cartesian.ne], union(a,b) );
    clear a, clear b;
    
    %find intersections
    for il = inter_element
        
        P2.x    = [Tau_cartesian.coords_element{il}(:,1);Tau_cartesian.coords_element{il}(1,1)];
        P2.y    = [Tau_cartesian.coords_element{il}(:,2);Tau_cartesian.coords_element{il}(1,2)];
        P2.hole = 0;
        
        Pintersect = PolygonClip(P1,P2,1);
        if(size(Pintersect,2)~=0)
            for npoly=1:size(Pintersect,2)
                P = Pintersect(npoly);
                if size(P,2)~=0
                    P.x = P.x(end:-1:1);
                    P.y = P.y(end:-1:1);
