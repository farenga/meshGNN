...
function [DATI]=dati_2

DATI= struct(  'method',           'SIPG',...
               'domain',           [0,1;0,1],...              
               'exact_sol',        'sin(x*pi).*sin(y*pi)',...    
               'source',           '8',...%'2*pi^2*sin(x*pi).*sin(y*pi)',...
               'dirichlet',        '0',...
               'grad_exact_1',     'pi*cos(x*pi).*sin(y*pi)',...    
               'grad_exact_2',     'pi*sin(x*pi).*cos(y*pi)',...  
               'fem',               1,...   
               'penalty_coeff',     10,... 
               'nqn',               3,...
               'MGmethod',          'h');
...
%                'source',           '1',... %'-(x.^2+3*x).*exp(x).*(y.^2-y)-2*(x.^2-x).*exp(x)',...  '1',...
%                'dirichlet',        '0',...
%                'grad_exact_1',     '(x.^2+x-1).*(y.^2-y).*exp(x)',... %not true 
%                'grad_exact_2',     '(x.^2-x).*(2*y-1).*exp(x)',... %not true   
%                'fem',               1,...
%                'penalty_coeff',     10,...
%                'nqn',               3,...
%                'MGmethod',          'h',...
%                'permeability',      'EvenAndOdd');
