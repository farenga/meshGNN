function M_hH = Mixed_MassMatrix_QuadFree_2(femregion_H,femregion_h)
...
p = femregion_h.fem;
q = femregion_h.fem;
[P_coeff,dP] = get_coeff_Legendre1D(p);
[Q_coeff,dQ] = get_coeff_Legendre1D(q);

M_hH = sparse(femregion_h.ndof,femregion_H.ndof);

local_enumeration_basis_functions_p = zeros(femregion_h.nln,2);
idx = 1;
for ix = 0:p
    for iy = 0:p-ix
        local_enumeration_basis_functions_p(idx,1) = ix;
        local_enumeration_basis_functions_p(idx,2) = iy;
        idx = idx + 1;
    end
end

local_enumeration_basis_functions_q = zeros(femregion_H.nln,2);
idx = 1;
for ix = 0:q
    for iy = 0:q-ix
        local_enumeration_basis_functions_q(idx,1) = ix;
        local_enumeration_basis_functions_q(idx,2) = iy;
        idx = idx + 1;
    end
end
...
%                 end
                ...
                for idx = 0:q
                    for idy = 0:p
                        ...
                        for k = 0:idx+idy
                            ...
                            for i = 0:min(k,idx)
                                if 0 <= k-i && k-i <= idy
                                    Cxy1 = Cxy1 + Q_coeff(idx+1,i+1)*Ctilde1(idy+1,k-i+1);
                                    Cxy2 = Cxy2 + Q_coeff(idx+1,i+1)*Ctilde2(idy+1,k-i+1);
                                end
                            end
                ...
                for idI = 1:femregion_H.nln
                    ...
                    ix = local_enumeration_basis_functions_q(idI,1);
                    iy = local_enumeration_basis_functions_q(idI,2);
                    
                    for idJ = 1:femregion_h.nln
                        
                        % Phi_J(x,y) = phi_{jx}(x) * phi_{jy}(y)
                        jx = local_enumeration_basis_functions_p(idJ,1);
                        jy = local_enumeration_basis_functions_p(idJ,2);
