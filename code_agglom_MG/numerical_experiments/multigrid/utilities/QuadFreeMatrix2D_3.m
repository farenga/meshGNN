function [Matrices] = QuadFreeMatrix2D_3(femregion,neighbour,P,dP,Dati)
...
% 3) Partial sum of coefficients in reference element:
%     this routine receives P and dP, coefficient matrices such that:
...
% HM represents the mass matrix of the homogeneous functions x^k * y^l
% FHM is similar but it is related to face integration
...
for ie = 1:femregion.ne % loop over elements
    ...
    for iedg = 1:neighbour.nedges(ie) % loop over faces, neighbour.nedges(ie) is the number of edges of the element ie
        ...
        if neigh_ie(iedg) == -1 %GP: means that iedg is a boundary edge of ie
            % Folloing Cangiani, Manolis and Houston:
            %penalty_scaled = femregion.rho(ie)*penalty_coeff*Cinv(iedg)*meshsize(iedg)/femregion.area(ie);
            
            % Folloing Dryja:
            hij = femregion.h(ie);
            rhoij = femregion.rho(ie);
            penalty_scaled = penalty_coeff*rhoij/hij;
        else
            % Folloing Cangiani, Manolis and Houston:
            %Cinv_ext = femregion.area(neigh_ie(iedg))./femregion.max_kb{neigh_ie(iedg)}(neigedge);
            %s1 = femregion.rho(ie)*penalty_coeff*Cinv(iedg)*meshsize(iedg)/femregion.area(ie);
            %s2 = femregion.rho(neigh_ie(iedg))*penalty_coeff*Cinv_ext*meshsize(iedg)/femregion.area(neigh_ie(iedg));
            %penalty_scaled = max([s1 s2]);
            %penalty_scaled = 2*s1*s2/(s1 + s2);
            
            % Folloing Dryja:
            hij = harmonic_average(femregion.h(ie),femregion.h(neigh_ie(iedg)));
            rhoij = harmonic_average(femregion.rho(ie),femregion.rho(neigh_ie(iedg)));
            penalty_scaled = penalty_coeff*rhoij/hij;
        end
        %%%%%%%%%%%%%%%%%%%%
        ...
        for idI = 1:femregion.nln
            ...
            for idJ = 1:femregion.nln
                ...
                % Folloing Cangiani, Manolis and Houston:
                %if neigh_ie(iedg) ~= -1 % internal faces
                %    I(index(idI),index(idJ)) = I(index(idI),index(idJ)) +  0.5 * femregion.rho(ie) * Int_I;
                %    IN(idI,idJ,iedg) = IN(idI,idJ,iedg) -  0.5 * femregion.rho(ie) *Int_IN;
                %    SN(idI,idJ,iedg) = SN(idI,idJ,iedg) - penalty_scaled *Int_SN;
                %elseif neigh_ie(iedg) == -1 % boundary faces
                %    I(index(idI),index(idJ)) = I(index(idI),index(idJ)) + femregion.rho(ie) * Int_I;
                %end
                
                % Folloing Dryja:
                if neigh_ie(iedg) ~= -1 % internal faces
                    I(index(idI),index(idJ)) = I(index(idI),index(idJ)) +  0.5 * rhoij * Int_I;
                    IN(idI,idJ,iedg) = IN(idI,idJ,iedg) -  0.5 * rhoij *Int_IN;
                    SN(idI,idJ,iedg) = SN(idI,idJ,iedg) - penalty_scaled *Int_SN;
                elseif neigh_ie(iedg) == -1 % boundary faces
                    I(index(idI),index(idJ)) = I(index(idI),index(idJ)) + rhoij * Int_I;
                end
    ...
    for idI = 1:femregion.nln
        ...
        for idJ = idI:femregion.nln % and then consider the symmetry of M and V
            ...
            V(index(idI),index(idJ)) = femregion.rho(ie)*I_stif;
            
            % thanks to the symmetry of M and V
            M(index(idJ),index(idI)) = I_mass;
            V(index(idJ),index(idI)) = femregion.rho(ie)*I_stif;
