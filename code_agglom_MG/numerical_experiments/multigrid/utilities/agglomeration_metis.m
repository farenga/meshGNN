function [region_coarse] = agglomeration_metis(region,neighbour,n_elements_coarse)
...
epart = metismex('PartGraphKway',A,n_elements_coarse);
epart = epart+1;
t = sort(unique(epart));
t_check = [1:max(epart)];
...
% Check collinearity: comment this if you want hanging nodes for your mesh
% nodes_ID = [];
...
% Renumbering: comment if you want hanging nodes
...
% if you don't want hanging nodes then...
% for i = 1:nparts
...
%
% ... else...
region_coarse.connectivity=region_temp.connectivity;

region_coarse.coord = nodes;
