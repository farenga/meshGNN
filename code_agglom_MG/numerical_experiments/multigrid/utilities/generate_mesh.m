...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [region] = generate_mesh(Domain,NElem,MaxIter,P)
...
ne = length(Element);
Nnodes = size(Node,1);
modify = zeros(Nnodes,1);
for i = 1:Nnodes-1
    for j=i+1:Nnodes
        if( sqrt( (Node(i,1)-Node(j,1))^2 + (Node(i,2)-Node(j,2))^2) < toll )
            modify(j) = i;
        end
    end
end
...
for k=1:Nnodes
    if(modify(k)>0)
        for ie=1:ne
            ...
            if(Element{ie}(end)==Element{ie}(1))
                I = [I, length(Element{ie})];
            end
            Element{ie}(I) = [];
        end
    end
