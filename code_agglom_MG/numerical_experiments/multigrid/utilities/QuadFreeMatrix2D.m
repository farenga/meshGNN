function [Matrices] = QuadFreeMatrix2D(femregion,neighbour,P,dP,Dati)
...
% 4) This is for the evaluation of the forcing term, lambda and the matrix L
%    characterize the integration on each element 
%    - L is Vandermorde like matrix
%    - Mhf is the number of homogeneous function that you want to use for the
%      integration of f(x,y) * x^k * y^l: by choosing Mhf = 2*p, we are saying that 
%      f(x,y) * x^k * y^l  ~  \sum_{i+j <= 2*p} { C_ij * x^i * y^j} 
%
%    WARNING:
%    we are using the formula described in section 5.6 of the paper
%    "Numerical integration of homogeneous functions on convex and
%    nonconvex polygons and polyhedra", Eric B. Chin and Jean B. Lasserre
%    and N. Sukumar. 
%    There the formula is given for polynomials, and it is not clear how
%    that formula works for general functions!!!
Mhf = 2*p;
[nodes_1D, w_1D, nodes_2D, w_2D] = quadrature(Mhf+1);
lambda = nodes_1D;
d = 2;
for k = 0:Mhf
    for j = 0:Mhf
        L(k+1,j+1) = (lambda(k+1)^j) * (d + j);
    end
end
[Lower,Upper,Precond] = lu(L);

% 5) matrix of Newton's binomials
for i = 0:p
    for j = 0:i
...
for ie = 1:femregion.ne % loop over elements
    ...
    % From here ap to line 252, we use the formula described in section 5.6 
    % of the paper "Numerical integration of homogeneous functions on convex and
    % nonconvex polygons and polyhedra", Eric B. Chin and Jean B. Lasserre
    % and N. Sukumar, in order to evaluate 
    %     \int_{\kappa^} f(F(x^,y^)) * (x^)^k * (y^)^l * Jdet * dx^ * dy^;  
    % Interpolation values for the forcing term, and costruction of the
    % "spider-web" quadrature rule (nodes and weigths):
    % P.S. the spider-web quadrature needs the coordinates of the mass
    % center point of vhat, but here we set that point to be [0,0] for semplicity.
    Nface = size(vhat,2);
    F = zeros(Nface*(p+1),Mhf+1);
    pphys_1D_tot = [];
    w_1D_tot = [];
    for i = 1:Nface
        
        if i < Nface
            x1 = vhat(1,i); x2 = vhat(1,i+1);
            y1 = vhat(2,i); y2 = vhat(2,i+1);
        else
            x1 = vhat(1,i); x2 = vhat(1,1);
            y1 = vhat(2,i); y2 = vhat(2,1);
        end
        bi_x_hi = ((y2-y1)*x1 + (x1-x2)*y1);
        
        p1 = [x1,y1]; p2 = [x2,y2];
        p3 = 0.5*(p1+p2);
        vec = [p1;p2;p3];
        pphys_1D = get_jacobian_physical_points_faces(vec, nodes_1D);
        NNode = length(pphys_1D(:,1));
        
        % spider-web quadrature rule on vhat 
        pphys_1D_tot = [pphys_1D_tot; pphys_1D];
        w_1D_tot = [w_1D_tot, (bi_x_hi) * Jdet * w_1D];
        
        % evaluation of f on the points of the quadrature rule
        for k = 0:Mhf
            
            for n = 1:NNode
                x = BJ(1,1)*(lambda(k+1)*pphys_1D(n,1)) + trans(1); 
                y = BJ(2,2)*(lambda(k+1)*pphys_1D(n,2)) + trans(2);
                F((i-1)*NNode + n,k+1) = eval(Dati.source);
            end
            
        end
    end
    
    % Modified Homoenenous mass matrix on \Omega^, with the forcing term
    ForcingHM = [];
    for n = 0:p
        for m = 0:p-n
            ...
            rhs = zeros(Mhf+1,1);
            for k = 0:Mhf
                for r = 1:length(pphys_1D_tot(:,1))
                    
                    %eval shape function
                    x = lambda(k+1)*pphys_1D_tot(r,1);
                    y = lambda(k+1)*pphys_1D_tot(r,2);
                    
                    rhs(k+1) = rhs(k+1) + F(r,k+1) * x^n * y^m * w_1D_tot(r);
                end
            end
            Y_vec = Lower\(Precond*rhs);
            I_vec = Upper\Y_vec;
            ForcingHM(n+1,m+1) = sum(I_vec);
    ...
    for idI = 1:femregion.nln
        ...
        for k = 0:ix
            for l = 0:iy
                f(index(idI)) = f(index(idI)) + P(ix+1,k+1)*P(iy+1,l+1)*ForcingHM(k+1,l+1);
            end
        end
    ...
    for iedg = 1:neighbour.nedges(ie) % loop over faces, neighbour.nedges(ie) is the number of edges of the element ie
        ...
        % @ Paul: I will give you a pdf to describe the following
        % instructions
            ...
            for idJ = 1:femregion.nln
                ...
                for k = 0:ix+jx
                    for l = 0:iy+jy
                        ...
                        Int_I = Int_I + (Ca + Cb) * JdetFace * FHM(k+1,l+1,iedg);
                ...
                if neigh_ie(iedg) ~= -1
                    for k = 0:ix+jx
                        for l = 0:iy+jy
                            ...
                            Int_IN = Int_IN + (Ca + Cb) * JdetFace * FHM(k+1,l+1,iedg);
